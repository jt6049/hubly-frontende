...
import { getMembers, getMessages, messageToUser, sendMessage, updateChatStatus } from "../services";
...
export default function ContactCenter() {
  ...
  const sendMsgHandler = async (e) => {
        ...
        try {
          ...
          const response = await sendMessage({ data: messageData }, chatRooms[activel]._id);
...
console.log(status)
  ...
  return (
    <div style={{ margin: "2vh 0 0 1.5vw", color: "#6A6B70", width: "100%", display: "flex" }}>
        ...
        {chatRooms.map((room, index) => (
          <div key={room._id} className="chats">
            ...
            >
              ...
              <p className="msg">{room.messages?.at(-1)?.message || "No messages yet"}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {chatRooms.length > 0 && (
        <div key={chatRooms[activel]._id} className="secondpart">
          <div className="headingll">
            ...
            <GoHome size="25px" style={{ float: "right", margin: "0.6vh 1vw ",position:"fixed", top:"5px",right:"480px", }} />
            {chatRooms[activel].status==="resolved"?<p  style={{  height: "70vh", 
                    overflowY: "auto",
                    padding: "10px",
                    marginTop: "50px",
                    border: "1px solid #ddd",
                    borderRadius: "8px",
                    backgroundColor: "#f9f9f9"}}>the chat is closed</p>:
            <div>
            <div style={{  height: "70vh", 
              ...
              {chatRooms[activel].messages.map((msg, index) => (
                <div key={index}>
                  <h5 className="chatheader">{msg.senderModel === "User" ? "You" : `Chat ${activel + 1}`}</h5>
                  <p className="msg">{msg.message}</p>
                </div>
              ))}
            </div>
            <input type="text" placeholder="Type here" className="sendinput" value={
                messageData.message
      ...
      {chatRooms.length > 0 && (
        <div className="thirdpart">
          ...
          <div className="details">
            {member.map((item) => (
              <div key={item._id} className="detail">{item.firstName}</div>
            ))}
           
            <select className="detail" style={{
                width:"26vw",
                height:"4vh",
            }}
          ...
          <div style={{
            ...
            padding:"1vh 1vw"}}>
                ...
                <button style={{
                    ...
                    color:"white",
                    float:"right"
                }} onClick={statusHandler}>Confirm</button>
            </div>
        </div>
      )}
    </div>
